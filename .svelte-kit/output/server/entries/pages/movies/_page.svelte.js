...
const movies_json = [
  ...
  {
    title: "Climax",
    description: "French dancers gather in a remote, empty school building to rehearse on a wintry night. The all-night celebration morphs into a hallucinatory nightmare when they learn their sangria is laced with LSD.",
  ...
  {
    title: "Dzień Świra",
    description: "An ironic look at a climber who decides to do anything to get to the top.",
  ...
  },
  {
    ...
    comment: "One of the movies actually touching on the todays society problems.",
    rating: 1
    /* NEUTRAL */
  },
  {
  ...
  },
  {
    ...
    description: "John Wilson embarks on an odyssey of self-discovery and cultural observation by covertly filming the lives of fellow New Yorkers while trying to give everyday advice.",
  ...
  },
  {
    ...
    comment: "I cannot recall any solid big-picture from the movie, even though I recall some of the scenes.",
    rating: 0
    /* REWATCHABLE */
  },
  {
...
function _page($$payload, $$props) {
  push();
  const each_array = ensure_array_like(movies_json);
  head($$payload, ($$payload2) => {
    $$payload2.title = "<title>Simon Gracki | movies</title>";
  ...
  for (let i = 0, $$length = each_array.length; i < $$length; i++) {
    ...
    $$payload.out += "<div><h5>".concat(escape_html(movie.title), '</h5> <p class="text-secondary truncate">').concat(escape_html(movie.comment), "</p></div>");
  }
  $$payload.out += "<!--]--></div>";
  pop();
}
